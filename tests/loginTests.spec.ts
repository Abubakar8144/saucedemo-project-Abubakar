...
test.describe('Login Page Tests', () => {
    ...
    test('Successful login', async ({ page }) => {
        ...
        //await expect(page).toHaveURL(fixturesData.inventoryURL)
        await pageManager.getLoginPage().validateCurrentUrl(page,fixturesData.inventoryURL)
    });
    ...
    test('Login with invalid credentials', async () => {
        ...
        expect(errorMessage).toContain(fixturesData.invalid_credentials_error_message)
    });

    test('Empty username error', async () => {
        await pageManager.getLoginPage().login('', fixturesData.valid_password);
        const errorMessage = await pageManager.getLoginPage().getErrorMessage();
        expect(errorMessage).toContain(fixturesData.empty_username_error_message);
    });

    test('Empty password error', async () => {
        await pageManager.getLoginPage().login(fixturesData.valid_username, '');
        const errorMessage = await pageManager.getLoginPage().getErrorMessage();
        expect(errorMessage).toContain(fixturesData.empty_password_error_message);
    });
});
